jQuery(document).ready(function ($) {
  ...
  if (priceToggle) {
	...
	//   CF7 Multistep Form
	function initCf7MultiStepForm() {
		const form = document.querySelector('.multistep-form');
		if (!form || form.classList.contains('multistep-initialized')) return;

		form.classList.add('multistep-initialized');

		const fieldsets = form.querySelectorAll('fieldset');
		const nextBtns = form.querySelectorAll('.next');
		const prevBtns = form.querySelectorAll('.previous');
		const progressBar = form.querySelectorAll('#progressbar li');
		let currentStep = 0;
	...
	document.addEventListener('wpcf7spam', function (event) {
		initCf7MultiStepForm();
	}, false);

	initCf7MultiStepForm(); 
//   END CF7 Multistep Form
});